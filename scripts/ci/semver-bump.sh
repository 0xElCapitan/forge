...
main() {
  ...
  # ---------------------------------------------------------------------------
  # Skip-release marker check (cycle-002 forge-1bo migration)
  # ---------------------------------------------------------------------------
  # Scan commits between $tag_ref and HEAD for opt-out markers in subject or
  # body. If found, emit empty JSON and exit 0 — the post-merge workflow's
  # `simple-release` job interprets empty/missing `.next` as "skip tag", so
  # no software tag is created on this merge.
  #
  # Recognized markers (case-sensitive, anywhere in commit subject or body):
  #   [skip release]      - generic opt-out
  #   [skip ci-release]   - explicit CI-repair opt-out
  #   [no-bump]           - alternative form
  #
  # Use cases:
  #   - CI infrastructure repair PRs (workflows, scripts) that should not
  #     trigger a software version bump.
  #   - Documentation-only changes the operator wants tag-suppressed.
  #   - Reverts or fix-ups where automatic tagging would create noise.
  #
  # The marker MUST appear in at least one commit between the latest tag and
  # HEAD. Because the post-merge workflow checks out the merge commit and
  # tag_ref is the latest semver tag, the entire PR commit range is in scope.
  # Including the merge commit subject itself — if `gh pr merge --merge
  # --subject "... [skip release]"` is used, that subject is matched too.
  #
  # NOTE: process substitution (`< <(...)`) is used instead of `|` because
  # this script runs under `set -euo pipefail`. With a pipeline, `grep -q`
  # exits immediately on first match, which closes its stdin; the upstream
  # `git log` then dies with SIGPIPE (exit 141), and `pipefail` propagates
  # that non-zero exit, making the `if` condition evaluate FALSE even though
  # the marker was found. Process substitution sidesteps the pipeline so
  # only grep's exit code is tested.
  if grep -qE '\[skip release\]|\[skip ci-release\]|\[no-bump\]' \
      < <(git -C "$PROJECT_ROOT" log "${tag_ref}..HEAD" --format='%B' 2>/dev/null); then
    echo "[semver-bump] skip-release marker found in commits between ${tag_ref} and HEAD — emitting empty JSON to suppress tag" >&2
    echo '{}'
    exit 0
  fi
