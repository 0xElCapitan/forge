...
test('self-consistency: the REAL discovered lab specs equal the REAL inventory exactly', async () => {
  ...
  assert.equal(registered.length, 38, 'thirty-eight lab specs (fourteen S02/S03 census/freeze + eleven Cycle-005 S01 acquisition/resolution + one F1 review-remediation spec + one pre-Gate-A credential-hardening spec + one pre-G0 acquisition-CLI spec + one pre-G0 F1 path-parameter-authority spec + one pre-G0 C-5/C-6 transport-authority spec + one S02 T2.2 pre-contact pin-invariance spec + one S02 composition-root spec + four Cycle-006 S01 LU-1 specs: the branch-aware evidence layer, the CI-line derivation harness, the lab/survey lane fences, and the SP-F3 landing guard + one Cycle-006 S01 LU-2A spec: the survey tooling surface + one Sprint-6 spec: the Gate-P revision-2 re-issue surface)');
});
