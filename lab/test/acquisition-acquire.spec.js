...
import { verifyAcquisitionIdentity, companionDigestPath, AcquisitionRefusal as IdentityRefusal } from '../resolution/identity.js';
...
// DR-12.3). This canary formerly ran BOTH lanes over the LIVE tree and asserted they
// agreed with a fresh recompute. Cycle-006 edits the apparatus IN PLACE under shape α to
// a NEW identity, so from the first α edit both lanes REFUSE the live tree against the
// Cycle-005 manifest — by design (C6-FR-A4/B5), not by defect. The lanes' equivalence
// over the ACCEPTED record is therefore asserted from COMMITTED BYTES: the two lanes'
// digest primitives are the same function (proven below), so agreeing on the committed
// manifest bytes is agreement on the accepted identity. No live-tree recompute remains.
// The synthetic-repo equivalence and drift-refusal tests above are unchanged and still
// exercise both lanes end-to-end over a tree that matches its manifest.
test('F1/FR-A4: Lane A and Lane B agree over the accepted Gate-A manifest FROM ITS BYTES', () => {
  ...
  // Lane A's `sha256LF` and Lane B's `sha256LFNormalized` are the identical primitive,
  // so both lanes derive the same companion from the same committed bytes.
  assert.equal(sha256LF(bytes), sha256LFNormalized(bytes), 'both lanes digest the committed manifest identically');
  assert.equal(readFileSync(companionDigestPath(manifestPath), 'utf8').trim(), sha256LFNormalized(bytes), 'the committed companion is that digest');
  assert.equal(sha256LFNormalized(bytes), 'sha256:073c7ff2d5acb6db548bddd34e34de4e65eaeaaff31a6051cdc05ccfddb8e4c6', 'the accepted Cycle-005 apparatus identity');
  assert.equal(JSON.parse(bytes).assets.length, 23);
});
